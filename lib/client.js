...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ---------------------------------------------------------- notification
    function notifyKind(kind, st) {
      try {
        if (typeof window.Notification === "undefined") return;
        const valley = kind === "valley";
        const title = valley ? "低谷时段开始 · 半价！" : "高峰时段开始 · 恢复原价";
        let body;
        if (valley) {
          body = "现在跑任务半价优惠，冲！距高峰还有 " + fmtDur((st.endMin - st.mins) * 60) + "。";
        } else {
          const eh = Math.floor(st.endMin / 60) % 24;
          const em = st.endMin % 60;
          body = "价格已恢复 ×2.00，省钱请等下一个低谷（" + pad2(eh) + ":" + pad2(em) + " 开始）。";
        }
    ...
    function AlarmToast(props) {
      ...
      // Unlock WebAudio on the first interaction (autoplay policy).
      React.useEffect(() => {
      ...
      const sub = valley
        ? "半价优惠进行中 · 距高峰还有 " + fmtDur((st.endMin - st.mins) * 60)
        : "价格恢复 ×2.00 · 下一个低谷 " + pad2(Math.floor(st.endMin / 60) % 24) + ":" + pad2(st.endMin % 60) + " 开始";
