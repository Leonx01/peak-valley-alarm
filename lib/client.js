...
// A corner badge (bottom-right) shows the DeepSeek API remaining balance
// (fetched through the host proxy /peak-valley-alarm/balance, so the API key
// never reaches the browser) plus the current input/output token prices.
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      ".pva-balance{position:fixed;right:16px;bottom:16px;z-index:2147482900;display:flex;flex-direction:column;gap:3px;padding:8px 12px;border-radius:12px;background:linear-gradient(150deg,rgba(250,247,239,.94),rgba(242,235,220,.9));border:1px solid rgba(176,142,78,.5);box-shadow:0 8px 24px rgba(80,55,10,.16);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);font-family:'Noto Serif SC','Source Han Serif SC','Songti SC','STSong','SimSun',serif;color:#3b3324;user-select:none;cursor:pointer;font-size:11px;line-height:1.55;letter-spacing:.06em}",
      "body[data-ds-dark-theme] .pva-balance{background:linear-gradient(150deg,rgba(31,35,41,.92),rgba(21,24,29,.9));border-color:rgba(196,168,106,.45);color:#eae3d3;box-shadow:0 8px 24px rgba(0,0,0,.4)}",
      ".pva-balance:hover{border-color:rgba(176,142,78,.85)}",
      ".pva-balance-row{display:flex;align-items:baseline;gap:6px;white-space:nowrap}",
      ".pva-balance-label{opacity:.62;font-size:10px;letter-spacing:.14em}",
      ".pva-balance-val{font-weight:700}",
      ".pva-balance-refresh{opacity:.45;font-size:10px}"
    ].join("\n");
    ...
    function AlarmToast(props) {
      ...
      const fire = React.useCallback(
        (kind, st) => {
          ...
          if (notify) notifyKind(kind, st, priceLine(prices, kind));
        },
        [sound, notify, prices]
      );
    ...
    // ------------------------------------------------------------ balance UI
    // Corner badge: DeepSeek API remaining balance (host proxy) + the current
    // input/output token prices for the live 峰/谷 state. Click to refresh.
    function BalanceInfo(props) {
      ...
      React.useEffect(() => {
        ...
        const t = setInterval(refresh, refreshMinutes * 60 * 1000);
        return () => clearInterval(t);
      }, [refresh, refreshMinutes]);

      // Re-render every second so the displayed price follows 峰/谷 flips.
      React.useEffect(() => {
        const t = setInterval(() => setNow(Date.now()), 1000);
        return () => clearInterval(t);
      }, []);
      ...
      return h(
        "div",
        { className: "pva-balance", title: "点击刷新余额", onClick: refresh },
        showBalance
          ? h(
              "div",
              { className: "pva-balance-row" },
              h("span", { className: "pva-balance-label" }, "余额"),
              h("span", { className: "pva-balance-val" }, balanceVal),
              refreshing ? h("span", { className: "pva-balance-refresh" }, "…") : null
            )
          : null,
        h(
          "div",
          { className: "pva-balance-row" },
          h("span", { className: "pva-balance-label" }, "当前价"),
          h("span", { className: "pva-balance-val" }, priceLine(prices, st.kind))
        )
      );
    }
